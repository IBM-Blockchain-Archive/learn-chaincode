...
func (t *SimpleChaincode) writeBitgramIdentity(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	marblesAsBytes, err := stub.GetState(bitgramIndexStr)
	if err != nil {
	...
	err = stub.PutState(bitgramIndexStr, jsonAsBytes)						//store name of marble
...
func (t *SimpleChaincode) open_trade(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	tradesAsBytes, err := stub.GetState(bitgramTradesStr)
	if err != nil {
	...
	err = stub.PutState(bitgramTradesStr, jsonAsBytes)								//rewrite open orders
	if err != nil {
		return nil, err
	}
